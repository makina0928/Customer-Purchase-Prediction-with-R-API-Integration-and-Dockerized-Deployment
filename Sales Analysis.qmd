...
- **Loading and Checking Data types**
...
* **Typical problems revealed by data types** 

In the date column, it is reviewed that the dates are in string format '<chr>'. Let us utilize `lubridate` R-package to address this issue. Another issue noticed in the data is the user name, we need to combine first name and last name columns. 
...
-   **Create a subset of our data frame**
We need to select, explore and analyse only the columns needed to achieve our objective.

```{r}
library(gt)
Data <-
select(df2,
 order_date, status, item_id, sku, qty_ordered, price, value, discount_amount, total, category, payment_method, Gender, age, full_name,`Place Name`, County, City, State, Region, Discount_Percent)
...
```
# 3. Spotting problems using graphics and visualization
